...
class Phi2Sven(BaseModel):

    def __init__(self, api_key=None) -> None:
        self.tokenizer = AutoTokenizer.from_pretrained("models/trained/phi-2-lmsys/checkpoint-last")
        self.model = PhiPrefix.from_pretrained("models/trained/phi-2-lmsys/checkpoint-last", device_map="auto", vocab_size=len(self.tokenizer))
        prefix_file = os.path.join("models/trained/phi-2-lmsys-sven/checkpoint-last", 'pytorch_model.bin')
        self.model.prefix_params.load_state_dict(torch.load(prefix_file))
