...
class PhiPrefix(PhiForCausalLM):
    def __init__(self, config):
        ...
        # Two control IDs, two tensors per layer (key + value)
        for _ in range(2):
    ...
    def get_past_from_prefix(self, control_ids):
        """
        Build a DynamicCache from prefix parameters.
        """
        batch_size = len(control_ids)
        device = self.prefix_params[0].device
        dtype = self.prefix_params[0].dtype
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            key_tensor = torch.stack(key_stack).to(device=device, dtype=dtype)
            val_tensor = torch.stack(val_stack).to(device=device, dtype=dtype)

            past.update(key_tensor, val_tensor, layer_idx=i)
